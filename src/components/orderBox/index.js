...
function OrderBox({
  ...
  const totalPrice = cakes
    .reduce((total, acc) => {
      return total + acc.price * acc.quantity;
    }, priceFlower)
    .toFixed(2);

  const totalQte = cakes
    .reduce((total, acc) => {
  ...
  const pickUpTheBox = () => {
    if (cakes.length > 0) {
      if (user) {
        if (box) {
          firebase.db
            .ref(`users/${user.uid}/boxes/${box.id}`)
            .update({
              cakes,
              numCake,
              flowers,
              total: totalPrice,
              image:
                numCake === 3 ? boxFor3 : numCake === 6 ? boxFor6 : boxFor9,
            })
            .then(() => {
              editBox(null);
    ...
    <Container>
      ...
      {flowers && (
        <Order
          ...
          qte
          qteNum={1}
          handlerDelete={() => setflowers(false)}
        />
      )}
      {totalQte > numCake && (
        <Error>your order is exceeded limit of the box {numCake}</Error>
      )}
      <TotalOrder>
        <Title>total: ${totalPrice}</Title>
        {!box && (
          <Btn disabled={totalQte > numCake} onClick={pickUpTheBox}>
            pick up the box
          </Btn>
        )}
      </TotalOrder>
      {box && (
        <OrderBox.Edit
          history={history}
          editBox={editBox}
          pickUpTheBox={pickUpTheBox}
        />
      )}
    </Container>
  );
}
OrderBox.Edit = function OrderBoxEdit({
  totalQte,
  numCake,
  history,
  editBox,
  ...
  return (
    <Edit>
      ...
      <Btn disabled={totalQte > numCake} onClick={cancelBox} color="true">
        cancel
      </Btn>
    </Edit>
  );
